...
headers = {"x-auth-token": "b540bd407852678c0af5b11105dcde14",
           "Content-Type": "application/x-www-form-urlencoded"}

list_of_courses = \
requests.get(f"https://userapi.webinar.ru/v3/organization/courses",
             headers=headers).json()["data"]

main_data_about_group = requests.get(
    f"https://userapi.webinar.ru/v3/organization/courses/groups",
...
def custom_requests(typee, *args):
    if typee == 0:
        return requests.get(
            f"https://userapi.webinar.ru/v3/courses/{args[0]}/groups/{args[1]}/statistics",
            headers=headers).json()
    elif typee == 1:
        return requests.get(
            f"https://userapi.webinar.ru/v3/contacts/{args[0]}/user",
            headers=headers).json()
    elif typee == 2:
        return requests.get(
            f"https://userapi.webinar.ru/v3/organization/users/{args[0]}/statistics",
            headers=headers).json()
...
make_files("courses", list_of_courses)
...
li = []
for i in main_data_about_group:
    data_aboit_group = custom_requests(0, i['course']['id'], i['id'])[0][
        "lessonsPassing"]
    for j in data_aboit_group:
        li.append({"id": j["id"], "name": j["name"], "type": j["type"],
                   "id_group": i["id"], "data": ""})
...
for i in main_data_about_group:
    ...
    for j in data_aboit_group:
        ...
        li.append({"id": userID, "firstName": data_about_student["firstName"],
                   "lastName": data_about_student["lastName"],
                   "email": data_about_student["email"]})
...
for i in main_data_about_group:
    ...
    for j in data_aboit_group:
        ...
        for c in j['lessonsPassing']:
            li.append({"userid": userID, "lessonid": c["id"],
                       "date": 1 if c["date"] else 0,
...
for i in main_data_about_group:
    ...
    for j in data_aboit_group:
        userID = custom_requests(1, j['contact']['id'])["id"]
        datareg = datetime.strptime(j['student']['registeredAt']['date'],
                                    '%Y-%m-%d %H:%M:%S.%f')
        datelast = datetime.strptime(j['student']['lastActivityAt']['date'],
                                     '%Y-%m-%d %H:%M:%S.%f')
        ...
        li.append({"user_id": userID, "group_id": i['id'], "datereg": datareg,
                   "datelast": datelast, "status": status,
                   "passingProgress": passingProgress,
                   "averageScore": averageScore})

make_files("user_group", li)
